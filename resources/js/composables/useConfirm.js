...
export const useConfirm = () => {
  ...
  const confirm = (options, slots = null) => {
    const { text, title, confirmText, cancelText, absolute, disablePadding, top = 0, selector } = options;
    confirmPromise.value = new Promise((resolve) => {
      ...
      const confirmComponent = defineComponent(ModalConfirm);
      const slotElements = {};
      if (slots) {
      ...
      const vnode = createVNode(confirmComponent, { title, text, confirmText, cancelText, absolute: absolute || false, disablePadding: disablePadding || false, top }, slotElements);
