...
@section('content')
    <!-- /.card-header -->
    <div class="card-body">
        <form action="{{ route('bet-edit',$bet->id) }}" id="create-bet-form" method="post"
              enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <label for="title">Input the bet's title</label>
                <input name="title" type="text" value="{{ $bet->title }}"
            ...
            <div class="row">
                ...
                <select name="status" id="status" class="form-select">
                    @foreach(\App\Http\Enums\BetStatusEnum::cases() as $status)
            ...
            <div class="row">
                ...
                <textarea name="description" type="text" class="form-control @error('description') is-invalid @enderror"
                          rows="5">{{ $bet->description }}</textarea>
            ...
            <div class="row">
                ...
                <input name="finish" type="datetime-local" value="{{ $bet->finish }}"
                       class="form-control @error('finish') is-invalid @enderror">
                @error('finish')<span class="text-danger fw-bold">{{ $message }}</span> @enderror
            </div>
            <div class="row">
                <label for="source1" class="form-label">Enter a link 1 to the source where the outcome of the dispute
                    will be indicated on the appointed date</label>
                <input name="source1" type="text" value="{{ $bet->source1 }}"
                       class="form-control @error('source1') is-invalid @enderror">
                @error('source1')<span class="text-danger fw-bold">{{ $message }}</span> @enderror
            </div>
            <div class="row">
                <label for="source2" class="form-label">Enter a link 2 to the source where the outcome of the dispute
                    will be indicated on the appointed date</label>
                <input name="source2" type="text" value="{{ $bet->source2 }}"
                       class="form-control @error('source2') is-invalid @enderror">
                @error('source2')<span class="text-danger fw-bold">{{ $message }}</span> @enderror
            </div>
            <div class="row">
                <label for="source3" class="form-label">Enter a link 3 to the source where the outcome of the dispute
                    will be indicated on the appointed date</label>
                <input name="source3" type="text" value="{{ $bet->source3 }}"
                       class="form-control @error('source3') is-invalid @enderror">
